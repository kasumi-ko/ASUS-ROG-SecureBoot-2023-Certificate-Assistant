...
# Capture the entry command before PS2EXE changes script path variables.
...
function Protect-AppDataDirectory {
    ...
    # Apply ACLs without following reparse points.
...
function Restart-Elevated {
    if ($script:IsCompiledExe) {
        # Stop PS2EXE from parsing application arguments.
...
# Create protected state storage after OOBE confirmation.
...
function Test-WritableDirectory {
    ...
    # Probe write access without recreating a missing saved folder.
    ...
    try {
        ...
        if ($targetExists) {
            ...
            if ($CreateIfMissing) {
                ...
                # Probe the nearest existing parent during OOBE.
...
function Export-DiagnosticSnapshot {
    ...
    try {
        ...
        try { Write-UiLog ((L '诊断报告写入失败：{0}' 'Diagnostic snapshot write failed: {0}') -f $_.Exception.Message) 'WARN' } catch {}
    }
}
...
function Update-SummaryFile {
    ...
    $lines = @(
        ...
        ((L 'Windows 启动管理器（Windows Boot Manager）首启动：{0}' 'Windows Boot Manager first: {0}') -f $State.BootChain.WindowsBootManagerFirst),
        ((L 'Windows 启动管理器（Windows Boot Manager）路径：{0}' 'Windows Boot Manager path: {0}') -f $State.BootChain.WindowsBootManagerPath),
        ((L '还原出厂密钥（Restore Factory Keys）风险等级：{0}' 'Restore Factory Keys risk level: {0}') -f (Get-DefaultResetRiskLevelDisplay $State.DefaultResetRiskLevel)),
...
function New-BootChainNotNeededState {
    ...
    $message = if ($SecureBootEnabled) {
        L '安全启动（Secure Boot）已启用。' 'Secure Boot is enabled.'
    } else {
...
function Get-BootChainState {
    ...
    if ((-not $fw.Succeeded -and -not $firmware.Succeeded) -or -not $bootmgr.Succeeded) {
        ...
        $result.Message = L '无法读取 Windows 固件启动项。启用安全启动（Secure Boot）前需要先确认当前启动项。' 'Unable to read Windows firmware boot entries. Check the current boot entry before enabling Secure Boot.'
        $result.DeepDiagnosticsMessage = $result.Message
        $result.ManualActionMessage = L '不要清除密钥（Clear Keys），也不要使用还原出厂密钥（Restore Factory Keys）。先以管理员身份运行 bcdedit /enum firmware。没有 Windows 启动管理器（Windows Boot Manager）时，运行 Windows 启动修复或 bcdboot %SystemRoot% /f UEFI，重启后点「重新检测」。' 'Do not clear Keys or use Restore Factory Keys. Run bcdedit /enum firmware as administrator. If Windows Boot Manager is missing, use Windows Startup Repair or run bcdboot %SystemRoot% /f UEFI, restart, and select Detect again.'
        $result.ManualReviewWorkflow = L '处理完成后重启，回到 Windows 点「重新检测」。检测通过后再进入 BIOS 开启安全启动（Secure Boot）。' 'Restart after fixing the issue, return to Windows, and select Detect again. Enable Secure Boot in BIOS after the check passes.'
    ...
    # Read the first firmware display-order identifier.
    ...
    if ($displayMatch.Success) {
        ...
        # Fall back to the full firmware list.
    ...
    $result.OfficialRotationEventSummary = ((L '最近安全启动（Secure Boot）更新事件数量：{0}。如 Windows 状态与证书检测不一致，请导出诊断报告。' 'Recent Secure Boot update event count: {0}. Export diagnostics if Windows status and certificate detection disagree.') -f $eventCount)
    ...
    # Repair only Windows Boot Manager order and path.
    ...
    if ($manualParts.Count -eq 0) {
        if ($result.RepairAvailable) {
            ...
            $manualParts += L '进入 BIOS 开启安全启动（Secure Boot）。不要清除密钥（Clear Keys），也不要使用还原出厂密钥（Restore Factory Keys）。' 'Enable Secure Boot in BIOS. Do not clear Keys or use Restore Factory Keys.'
        } else {
    ...
    $result.ManualReviewWorkflow = L '处理完成后重启，回到 Windows 点「重新检测」。检测通过后再进入 BIOS 开启安全启动（Secure Boot）。不要清除密钥（Clear Keys），也不要使用还原出厂密钥（Restore Factory Keys）。' 'Restart after fixing the issue, return to Windows, and select Detect again. Enable Secure Boot in BIOS after the check passes. Do not clear Keys or use Restore Factory Keys.'

    if ($result.IsSafeToEnableSecureBoot) {
        $result.RiskDisposition = L '可以进入 BIOS 开启安全启动（Secure Boot）' 'Secure Boot can be enabled in BIOS'
        ...
        $result.Message = L '未检测到可直接修复的 Windows 启动管理器（Windows Boot Manager）启动项。请先在 Windows 中修复启动项，再启用安全启动（Secure Boot）。' 'No repairable Windows Boot Manager entry was detected. Repair the Windows boot entry in Windows before enabling Secure Boot.'
    }
    return [pscustomobject]$result
}
...
function Repair-WindowsBootManagerOrder {
    ...
    if ($LASTEXITCODE -ne 0) { throw ((L '修复 Windows 启动管理器（Windows Boot Manager）路径失败：{0}' 'Failed to repair Windows Boot Manager path: {0}') -f $pathResult.Trim()) }
...
function Test-TransactionIntermediateState {
    ...
    try {
        ...
        if ($status -ne 'Active') {
            return [pscustomobject]@{ IsConsistent = $false; RecognizedStage = ''; Message = ((L '上次进度状态不是活动状态（Active）：{0}' 'The saved progress status is not Active: {0}') -f $status) }
        }
        ...
        if (-not $pk -and -not $kek -and -not $db -and -not $dbx) {
            return [pscustomobject]@{ IsConsistent = $true; RecognizedStage = 'NoKeysReady'; Message = L '设备和默认密钥（Default Keys）与记录一致，尚未开始写入。' 'The device and Default Keys match the saved record. No write has started.' }
        }
    ...
    return [pscustomobject]@{ IsConsistent = $false; RecognizedStage = ''; Message = L '当前活动密钥（Active Keys）与保存的进度不一致。' 'The current Active Keys do not match the saved progress.' }
}
...
function Get-SystemState {
    ...
    if (-not $isUefi) {
        ...
        $state.BlockReason = L '非华硕 / ROG 设备默认只读。' 'Non-ASUS/ROG devices use read-only detection by default.'
        ...
        $state.NextStep = L '固件变量读取失败。请导出诊断报告。' 'A firmware variable could not be read. Export diagnostics.'
        ...
        $state.NextStep = L '无需继续操作。不要使用还原出厂密钥（Restore Factory Keys）。' 'No further action is required. Do not use Restore Factory Keys.'
        ...
        $state.NextStep = L '当前设备或默认密钥（Default Keys）与未完成进度不一致。请重新开始或导入本机恢复文件。' 'The device or Default Keys do not match the unfinished progress. The old record cannot be reused.'
        $state.BlockReason = $state.TransactionConsistency.Message
    } elseif ($setupMode -eq 1 -and $secureBootVariable -eq 0 -and $noKeys -and -not $defaultVariablesReadable) {
        $state.Classification = 'FirmwareVariableReadFailure'
        $state.NextStep = L 'BIOS 默认密钥（Default Keys）读取异常。请导出诊断报告。' 'BIOS factory Keys returned an unexpected read error. Automated repair is blocked. Export diagnostics.'
        $state.BlockReason = L '至少一个 BIOS 默认密钥（Default Keys）无法可靠读取。' 'At least one BIOS factory Key could not be read reliably.'
        ...
        if ($activeNew -and (-not $state.BootChain.IsSafeToEnableSecureBoot)) {
            if ($state.BootChain.RepairAvailable) {
                ...
                $state.NextStep = L '当前活动密钥（Active Keys）已包含 2023 证书，但安全启动（Secure Boot）未启用。先将 Windows 启动管理器（Windows Boot Manager）修复为首启动项，再启用安全启动（Secure Boot）。' 'Active Keys already contain 2023 certificates, but Secure Boot is disabled. Repair Windows Boot Manager as the first boot entry before enabling Secure Boot.'
                $state.BootChainWarning = $state.BootChain.Message
            } else {
                $state.Classification = 'BootChainReviewRequired'
                $state.NextStep = L '当前活动密钥（Active Keys）已包含 2023 证书，但启动链检查未通过。按「操作」处理，重启后点「重新检测」。检测通过后再开启安全启动（Secure Boot）。不要清除密钥（Clear Keys）。' 'Active Keys contain the 2023 certificates, but the boot-chain check failed. Follow the listed steps, restart, and select Detect again. Enable Secure Boot after the check passes. Do not clear Keys.'
            ...
            $state.SecureBootEnableWarning = L 'Windows 启动管理器（Windows Boot Manager）检查通过。进入 BIOS 启用安全启动（Secure Boot）时，不要清除密钥（Clear Keys）。' 'The current Keys are present and the Windows Boot Manager boot-entry check did not find an issue requiring automatic repair. Do not clear the Keys when enabling Secure Boot in BIOS.'
            $state.BootChainWarning = $state.BootChain.Message
        }
    ...
    if (-not $defaultVariablesReadable) {
        ...
        $state.DefaultResetRisk = L 'BIOS 默认密钥（Default Keys）已包含 2023 证书。不要随意使用还原出厂密钥（Restore Factory Keys）。' 'BIOS factory Default Keys contain the 2023 certificates. Do not use Restore Factory Keys casually.'
    } else {
...
软件提示需要 设置模式（Setup Mode）时，再进 BIOS 清空 安全启动密钥（Secure Boot Keys）。
不要随意使用还原出厂密钥（Restore Factory Keys）。部分 BIOS 默认密钥（Default Keys）不包含完整 2023 证书，恢复后可能回到旧证书。
...
【文件】
选择日志和备份保存位置。
不要公开上传 默认密钥（Default Keys）原始备份、BitLocker 恢复密钥、个人文件，或包含设备备份的恢复文件。
'@
}
...
function Get-RecoveryStageFromEvidence {
    ...
    if (-not $pk -and -not $kek -and -not $db -and -not $dbx) {
        return [pscustomobject]@{ IsValid = $true; Stage = 'NoKeysReady'; Message = L '活动密钥（Active Keys）为空，可以从第一步开始。' 'Active Keys are empty. The process can start from step 1.' }
    }
    if ($db -and -not $dbx -and -not $kek -and -not $pk) {
...
当前记录无法证明之前的写入来源。请确认你使用的是本机保存的 默认密钥（Default Keys）备份和微软官方证书。
...
function New-AdvancedRecoveryTransaction {
    ...
    if (-not $state.IsAsus -or -not $state.IsUEFI) { throw (L '中断恢复需要华硕 / ROG 设备，并且 Windows 当前以 UEFI 模式启动。' 'Interrupted recovery requires an ASUS/ROG device with Windows booted in UEFI mode.') }
    if (-not $state.ActiveVariablesReadable -or -not $state.DefaultVariablesReadable -or -not $state.DefaultsAllReadable) { throw (L '活动密钥（Active Keys）或 BIOS 默认密钥（Default Keys）无法完整读取。' 'Active Keys or BIOS factory Keys cannot be read completely. Interruption recovery is blocked.') }
    if ($state.SetupMode -ne 1 -and -not $state.AllKeys) { throw (L '当前不是可验证的设置模式（Setup Mode）中间态。' 'The current state is not a verifiable Setup Mode intermediate state.') }
    ...
    foreach ($name in $nameMap.Keys) {
        ...
        if ($bytes.Length -ne $firmware.Length -or $hash -ne $firmware.Sha256 -or -not (Test-ByteArrayEqual -A $bytes -B ([byte[]]$firmware.Bytes))) {
            throw ((L '{0}备份与当前 BIOS 默认密钥（Default Keys）不一致。' 'The {0} backup does not exactly match the current BIOS factory Key.') -f $name)
        }
...
'@
    ...
    Write-UiLog ((L '恢复文件已保存：{0}。文件包含默认密钥（Default Keys）备份，请勿上传公开平台。' 'Recovery file saved: {0}. It contains Default Keys backups; do not upload it publicly.') -f $dialog.FileName) 'SUCCESS'
...
function Show-AdvancedRecoveryDialog {
    ...
    $advancedZh = @'
修复中断、记录丢失或只剩部分活动密钥（Active Keys）时使用。

请选择恢复文件，或选择四个默认密钥（Default Keys）备份和微软官方证书。验证通过后恢复进度，不写入固件。
'@
    $advancedEn = @'
...
'@
    ...
    $manual.Text = L '用默认密钥（Default Keys）备份验证当前进度…' 'Verify progress from Default Keys backups...'
...
function New-RepairTransaction {
    ...
    if (-not $State.DefaultsAllReadable) {
        throw (L '无法读取四个默认密钥（Default Keys），固件没有提供可用于写入的源数据。' 'All four Default Keys could not be read. The firmware did not provide source data for the write.')
    }
    $transactionId = [Guid]::NewGuid().ToString('N')
    $transactionRoot = Join-Path $script:BackupRoot (Join-Path 'Transactions' $transactionId)
    $keyRoot = Join-Path $transactionRoot 'KeyBackups'
    ...
    Save-Transaction $transaction
    Write-UiLog ((L '已创建进度记录 {0}，并备份四个 BIOS 默认密钥（Default Keys）。目录：{1}' 'Progress record {0} was created and all four BIOS factory Keys were backed up. Folder: {1}') -f $transactionId,$transactionRoot) 'SUCCESS'
    [Windows.Forms.MessageBox]::Show(((L '已创建修复进度和四个默认密钥（Default Keys）备份。保存位置：' 'The repair progress and four Default Keys backups were created at: ') + [Environment]::NewLine + $transactionRoot), $script:AppName, 'OK', 'Information') | Out-Null
    return $transaction
}
...
function Invoke-Append2023Certificate {
    ...
    # Revalidate the protected certificate copy before formatting db content.
...
function Register-ResumeTask {
    ...
    # Use the protected ProgramData runtime for the one-time resume task.
    if (-not (Test-Path -LiteralPath $script:ProgramPath)) { throw (L '找不到当前程序文件，无法创建续跑任务。' 'The current program file cannot be found. The resume task cannot be created.') }
    ...
    $sourcePath = [IO.Path]::GetFullPath($script:ProgramPath)
    $runtimePath = [IO.Path]::GetFullPath($script:ProtectedRuntimePath)
    $sameRuntimePath = [string]::Equals($sourcePath, $runtimePath, [StringComparison]::OrdinalIgnoreCase)
    $sourceHashBefore = Get-FileHashHex -Path $sourcePath -Algorithm SHA256
    if (-not $sameRuntimePath) {
        Copy-Item -LiteralPath $sourcePath -Destination $runtimePath -Force -ErrorAction Stop
    }
    Protect-AppDataDirectory -Path $script:AppDataRoot
    $sourceHashAfter = Get-FileHashHex -Path $sourcePath -Algorithm SHA256
    $runtimeHash = Get-FileHashHex -Path $runtimePath -Algorithm SHA256
    if ($sourceHashBefore -ne $sourceHashAfter -or $sourceHashAfter -ne $runtimeHash) {
        if (-not $sameRuntimePath) { Remove-Item -LiteralPath $runtimePath -Force -ErrorAction SilentlyContinue }
        throw (L '续跑程序校验失败，任务未创建。' 'Resume runtime verification failed. The task was not created.')
    }
...
function Invoke-RebootWithResume {
    ...
    if ($Destination -eq 'Firmware') {
        & "$env:SystemRoot\System32\shutdown.exe" /r /fw /t 5 /c "ASUS (ROG) Secure Boot 2023 Certificate Assistant: restart into UEFI."
    } else {
        & "$env:SystemRoot\System32\shutdown.exe" /r /t 5 /c "ASUS (ROG) Secure Boot 2023 Certificate Assistant: resume after restart."
    }
...
function Invoke-SafeUiAction {
    ...
    try {
        ...
        $userMessage = if ($script:Language -eq 'en-US') {
            "The operation stopped. Export the diagnostic report and include error ID $errorId."
        } else {
            $message
        }
...
function Show-SecureBootEnableGuidance {
    ...
    $message = if ([string]::IsNullOrWhiteSpace([string]$State.SecureBootEnableWarning)) {
        L '当前密钥已存在但安全启动（Secure Boot）未启用。进入 BIOS 启用安全启动（Secure Boot）时不要清除密钥（Clear Keys）。' 'The current Keys are present but Secure Boot is disabled. Do not clear the Keys when enabling Secure Boot in BIOS.'
    } else {
    ...
    if (Show-ConfirmationWarning -Title (L '启用安全启动（Secure Boot）' 'Enable Secure Boot') -Message $message) {
...
Windows 启动管理器（Windows Boot Manager）首启动：{1}
路径：{2}
可疑启动项：{3}

修复内容：Windows 启动管理器（Windows Boot Manager）启动顺序和标准路径。
...
Repair: Windows Boot Manager order and standard path.
...
'@
    ...
    if (-not (Show-ConfirmationWarning -Title (L '修复 Windows 启动管理器（Windows Boot Manager）' 'Repair Windows Boot Manager') -Message $message)) { return }
    Repair-WindowsBootManagerOrder
    Write-UiLog (L 'Windows 启动管理器（Windows Boot Manager）已修复。请重新检测后再启用安全启动（Secure Boot）。' 'Windows Boot Manager was repaired. Detect again before enabling Secure Boot.') 'SUCCESS'
}
...
处理完成后重启，回到 Windows 点「重新检测」。检测通过后再启用安全启动（Secure Boot）。不要清除密钥（Clear Keys），也不要使用还原出厂密钥（Restore Factory Keys）。
...
function Update-StateGrid {
    ...
    if ($showFullBootChainDetails) {
        ...
        $rows += ,@((L '启动链检查' 'Boot-chain check'),(L '安全启动（Secure Boot）已启用。' 'Secure Boot is enabled.'))
    }
...
function Update-OverviewGrid {
    ...
    if ($showBootChain) {
        ...
        [void]$rows.Add(@((L '操作' 'Steps'),$State.BootChain.ManualActionMessage))
    ...
    if (-not [string]::IsNullOrWhiteSpace([string]$State.SecureBootEnableWarning)) { [void]$rows.Add(@((L '启用安全启动（Secure Boot）提醒' 'Enable Secure Boot notice'),$State.SecureBootEnableWarning)) }
    [void]$rows.Add(@((L '风险等级' 'Risk level'),(Get-DefaultResetRiskLevelDisplay $State.DefaultResetRiskLevel)))
    [void]$rows.Add(@((L 'BIOS 默认密钥（Default Keys）说明' 'BIOS Default Keys details'),$State.DefaultResetRisk))
...
function Refresh-MainUi {
    ...
    try { Append-StateHistory -State $script:CurrentState -Reason $Reason } catch { try { Write-UiLog ((L '状态历史写入失败：{0}' 'State history write failed: {0}') -f $_.Exception.Message) 'WARN' } catch {} }
    try { Export-DiagnosticSnapshot -State $script:CurrentState -Reason $Reason } catch { try { Write-UiLog ((L '诊断报告写入失败：{0}' 'Diagnostic snapshot write failed: {0}') -f $_.Exception.Message) 'WARN' } catch {} }
    ...
    switch ($script:CurrentState.Classification) {
        ...
        'SecureBootDisabledWithKeys' {
            $script:PrimaryButton.Text = L '查看安全启动说明…' 'Read Secure Boot enable notice...'
            $script:PrimaryButton.Enabled = ([string]::IsNullOrWhiteSpace([string]$script:CurrentState.ActionBlockReason))
            $script:PrimaryButton.Visible = $true
        }
        'BootChainRepairRequired' {
            $script:PrimaryButton.Text = L '修复 Windows 启动管理器…' 'Repair Windows Boot Manager...'
            $script:PrimaryButton.Enabled = ([string]::IsNullOrWhiteSpace([string]$script:CurrentState.ActionBlockReason))
            $script:PrimaryButton.Visible = $true
        }
        ...
        'NeedsFirmwareSetup' {
            $script:PrimaryButton.Text = L '重启进入 BIOS 设置模式（Setup Mode）' 'Restart into UEFI to enter Setup Mode'
            $script:PrimaryButton.Enabled = ([string]::IsNullOrWhiteSpace([string]$script:CurrentState.ActionBlockReason))
            $script:PrimaryButton.Visible = $true
        }
        ...
        'Completed' {
            $script:NextActionLabel.Text = if ($script:CurrentState.DefaultResetRiskLevel -eq 'Warning') { L '已完成：无需继续操作。不要使用还原出厂密钥（Restore Factory Keys）。' 'Completed: no action is needed. Do not use Restore Factory Keys.' } else { L '已完成：无需继续操作。不要使用还原出厂密钥（Restore Factory Keys）。' 'Completed: no action is needed. Do not use Restore Factory Keys.' }
        }
    }
    ...
    if ($script:CurrentState.PostPkActiveStateVerified -and $null -ne $script:CurrentTransaction -and [string]$script:CurrentTransaction.Status -eq 'Locked') {
        Write-UiLog (L '已重新检测安全启动（Secure Boot）状态。' 'Secure Boot state was detected again.') 'WARN'
    }
}
...
不包含：默认密钥（Default Keys）原始备份、BitLocker 恢复密钥和个人文件。
...
function Resolve-DetectedPostPkReboot {
    ...
    $transactionStatus = [string]$script:CurrentTransaction.Status
    $pkStep = [string]$script:CurrentTransaction.Steps.PkDefault
    $lastError = [string](Get-OptionalPropertyValue -Object $script:CurrentTransaction -Name 'LastError' -Default '')
    ...
    $knownDelayedSetupMode = ($transactionStatus -eq 'Locked' -and $pkStep -eq 'Failed' -and $intentStep -eq 'PkDefault' -and $lastError -match 'SetupMode')
    if ($transactionStatus -ne 'Active' -and -not $knownDelayedSetupMode) { return }
    if ($pkStep -notin @('Pending','Complete') -and -not $knownDelayedSetupMode) { return }
    ...
    if ($knownDelayedSetupMode) { $script:CurrentTransaction.Status = 'Active' }
    try {
        $state = Get-SystemState
    } finally {
        if ($knownDelayedSetupMode) { $script:CurrentTransaction.Status = $transactionStatus }
    }
...
function Show-MainForm {
    ...
    $script:LanguageBox.Add_SelectedIndexChanged({
        ...
        $languageChangeMessage = L '切换语言后重新打开主界面。继续？' 'The main window reloads after the language changes. Current progress is unchanged. Continue?'
    ...
    $form.Add_Shown({
        ...
        if ($null -ne $script:CurrentTransaction -and $script:CurrentTransaction.Status -ne 'Complete') {
            ...
            Write-UiLog ((L '发现未完成进度 {0}。上次步骤：{1}。正在按当前 UEFI 状态检查进度。' 'Unfinished progress {0} was found. Previous step: {1}. Checking progress against the current UEFI state.') -f $script:CurrentTransaction.TransactionId,$pending) 'WARN'
        }
